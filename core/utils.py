...
def extract_domain_or_ip(target):
    """Extract the domain or IP address from a target URL."""
    try:
...
def is_ip(url):
    """Check if a URL contains an IP address as hostname."""
    try:
...
def normalize_target(target):
    ...
    if is_ip(target):
        ip = extract_domain_or_ip(target)
        if ip:
            parsed = urlparse(target)
            target = f"{parsed.scheme}://{ip}"
...
def find_forms(target, timeout=DEFAULT_TIMEOUT):
...
def get_csrf_token(session, target, timeout=DEFAULT_TIMEOUT):
...
def check_allowed_methods(target_url, timeout=DEFAULT_TIMEOUT):
    """Check which HTTP methods are allowed on a target URL."""
    session = create_session()
    try:
        get_response = session.get(target_url, timeout=timeout)
        post_response = session.post(target_url, timeout=timeout)
...
def detect_hidden_sqli_errors(response_text):
    """Detect SQL injection error signatures in response text."""
    SQLI_SIGNATURES = [
